...
void __fastcall TForm3::Button_OkClick(TObject *Sender)
{
	...
	if(MonthCalendar1->Date == StrToDate(date_RuToEng(p_list[1]->Strings[table.Y-1]))){
		Form3->Close();
		return;
	}
	...
	p_list[1]->Strings[table.Y-1] = FormatDateTime("dd.mm.yyyy", MonthCalendar1->Date);
	Form1->StringGrid1->Cells[3][table.Y] = FormatDateTime("dd.mm.yyyy",MonthCalendar1->Date);

	int diff=0;
	if((int)StrToDate(date_RuToEng(p_list[1]->Strings[table.Y-1])) != (int)(Date()) ){
		 if((int)StrToDate(date_RuToEng(p_list[1]->Strings[table.Y-1])) > (int)Date())
			while((int)StrToDate(date_RuToEng(p_list[1]->Strings[table.Y-1])) > ((int)Date() + ++diff)) ;
		 else
			while((int)StrToDate(date_RuToEng(p_list[1]->Strings[table.Y-1])) < ((int)Date() + --diff)) ;
	}
